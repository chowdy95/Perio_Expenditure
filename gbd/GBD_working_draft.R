...
trans_model <- transcan(~ perio_prop_oral + dent_pers_excl_dentists + dent_personnel + dent_exppc,
...
# Overall, the model using the transcan does not seem to have improved performance, nor did it reduce collinearity
...
# Variable clustering was then done to assess correlations between predictors
vc <- varclus(~ perio_prop_oral + dent_personnel + dentists + dent_exppc, data = GBD)
plot(vc)
vc

# 3. Optimal transformation of 4 selected predictors to account for skewed distribution of dentists, dent_personnel
# and dent_exppc
# Given the right skew, a sqrt transformation was used for dentists and dent_personnel to minimise the skew
# a log transformation was used for dent_exppc due to the greater skew

# No imputation necessary at this point as no missing data present for the countries with data on perio expenditure

GBD_trans <- GBD %>% mutate(
  sqrt_dent_personnel = sqrt(dent_personnel),
  sqrt_dentists = sqrt(dentists),
  log_dent_exppc = log(dent_exppc + 0.000000001)
)
# Added a very small number as there were countries with values of 0.0 for dent_exppc which
# would have caused an infinity result for log

dd <- rms::datadist(GBD_trans) # To update data distribution after log transformations
options(datadist = "dd")

d <- GBD_trans %>%
  select(log_dent_exppc, sqrt_dent_personnel, sqrt_dentists) %>%
  describe()

plot(d)
# The distribution appears to be much more symmetrically distributed

# 4. Linear regression

f5 <- ols(results ~ sqrt_dentists + sqrt_dent_personnel + log_dent_exppc, data = GBD_trans, x = TRUE, y = TRUE)
f5
# r2 and adjusted R2 were substantially poorer than previous models
...
# This diagnostic test shows residuals that are not normally distributed around the fitted values
plot(f5, which = 1)
# Residuals do not appear to be normally distributed

# This Q-Q plot does not demonstrate any clear abnormalities
r4 <- residuals(f5, type = "student")
qqnorm(r4)
qqline(r4)

# Compare this to the non-transformed model
f6 <- ols(results ~ dentists + dent_personnel + dent_exppc, data = GBD_trans, x = TRUE, y = TRUE)
f6
# The R2 and adjusted R2 seems significantly better for the non-transformed data
...
# Assessment of collinearity
vif(f6)
# Collinearity appears to be acceptable
ss
...
ggplot(temp, aes(x = rowname, y = dffits)) +
  geom_point() +
  geom_text(aes(label = GBD$rowname, vjust = -0.7))
# It appears that Norway is the most influential datapoint
