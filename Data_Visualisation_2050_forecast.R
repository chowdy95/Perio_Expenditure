...
plot_superregion_cost <- ggplot(data = superregion_cost_df, aes(x = Year, y = selected_Mean_total_billions, 
  ...
  geom_ribbon(aes(ymin = selected_Mean_total_billions - selected_SD_total_billions, 
              ...
              alpha = 0.05) +
  geom_line(aes(x = Year, y = transition_WHO_selected_Mean_total_billions,
                color = location_name, linetype = "WHO target of 80% coverage")) +
  geom_ribbon(aes(ymin = transition_WHO_selected_Mean_total_billions - transition_WHO_selected_SD_total_billions,
...
plot_global_cost <- ggplot(data = global_cost_df, aes(x = Year, y = selected_Mean_total_billions, 
                                                                group = location_name)) +
  geom_line(aes(color = location_name, linetype = "Current dental utilisation"))+
  geom_ribbon(aes(ymin = selected_Mean_total_billions - selected_SD_total_billions, 
                  ymax = selected_Mean_total_billions + selected_SD_total_billions, fill = location_name), 
...
ggsave("outputs_forecast/patchwork_global_superregion.pdf", plot_global_superregion, width = 18, height = 6, dpi = 300)
...
top_10_exppc_df <- perio_expenditure_countries %>%
  ...
  mutate(location_name = fct_rev(location_name))W
...
plot_top10_exppc_cost <- ggplot(top_10_exppc_df, aes(x = Year, y = selected_exppc_usd_mean, group = location_name)) +
  ...
  geom_ribbon(aes(ymin = selected_exppc_usd_mean - selected_exppc_usd_sd,
                  ymax = selected_exppc_usd_mean + selected_exppc_usd_sd, fill = location_name),
              alpha = 0.1) +
  geom_line(aes(x = Year, y = WHO_exppc_usd_mean,
                color = location_name, linetype = "WHO target of 80% coverage")) +
  geom_ribbon(aes(ymin = WHO_exppc_usd_mean - WHO_exppc_usd_sd,
